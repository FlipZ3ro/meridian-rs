...
mod tests {
    ...
    #[test]
    fn build_deploy_request_dual_side_carries_both_amounts_and_balanced_strategy() {
        let keypair = Keypair::new();
        let pool = solana_sdk::pubkey::Pubkey::new_unique().to_string();
        let config = crate::config::Config::default();
        ...
        .expect("dual-side deploy request should be built");

        assert_eq!(request.amount_x, 1_234_567);
        assert_eq!(request.amount_y, 250_000_000);
        assert_eq!(request.min_bin_id, 70);
        assert_eq!(request.max_bin_id, 130);
        assert_eq!(request.width, 61);
        assert_eq!(request.strategy, "spot_balanced");
    }

    /// The program rejects the wrong family outright (InvalidStrategyParameters,
    /// 0x17a6), so the mapping is pinned rather than left to inspection.
    #[test]
    fn strategy_type_follows_the_deposit_shape_not_the_strategy_name() {
        assert_eq!(
            strategy_type_from_name("spot", false),
            StrategyType::SpotImBalanced
        );
        assert_eq!(
            strategy_type_from_name("spot", true),
            StrategyType::SpotBalanced
        );
        assert_eq!(
            strategy_type_from_name("curve", false),
            StrategyType::CurveImBalanced
        );
        assert_eq!(
            strategy_type_from_name("curve", true),
            StrategyType::CurveBalanced
        );
        assert_eq!(
            strategy_type_from_name("bid-ask", false),
            StrategyType::BidAskImBalanced
        );
        assert_eq!(
            strategy_type_from_name("bid-ask", true),
            StrategyType::BidAskBalanced
        );
        // Unknown names fall back to spot, in whichever family applies.
        assert_eq!(
            strategy_type_from_name("nonsense", true),
            StrategyType::SpotBalanced
        );
    }
    ...
    #[tokio::test]
    async fn acquire_base_token_rejects_an_out_of_range_split() {
        let config = crate::config::Config::default();
        let mint = solana_sdk::pubkey::Pubkey::new_unique().to_string();

        // Guards before any RPC or swap: a bad split must never reach Jupiter.
        for bad_pct in [0.0, 0.01, 0.95, 1.0] {
            let err = acquire_base_token(&mint, 0.5, bad_pct, 100, &config, false)
                .await
                .expect_err("split outside 0.05..=0.9 should be refused");
            assert!(
                err.to_string().contains("dualSideBasePct"),
                "unexpected error: {}",
                err
            );
        }
